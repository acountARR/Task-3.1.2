...
async function fillPrincipalTable() {
    ...
    elem.nextElementSibling.innerHTML = user.roles.map(role => role.name.substr(5)).join(" ")
}
...
async function fillUsersTable() {
    ...
    for (const user of users) {
        ...
        tr.innerHTML = `
            ...
            <td>${user.age}</td>
            <td>${user.email}</td>
            <td>${user.roles.map(role => role.name.substr(5)).join(" ")}</td>
            <td>
                ...
                for (const option of field.options) {
                    if (option.selected) {
                        roles.forEach(function (role) {
                            if (role.name.substr(5) === (option.value)) {
                                val[field.name].push(
                                    {
                                        id: role.id,
                                        name: role.name
...
async function fillDeleteModal(id) {
    ...
    elem.innerHTML = user.roles.map(role => "<option>" + role.name.substr(5) + "</option>").join(" ")
}
